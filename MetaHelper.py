            ...
            try:
                ...
                dot_sublime_completions = {
                    ...
                    "completions": [
                        ...
                        "COUNT",
                        "Count(*)",
                        "CREATE MULTISET VOLATILE TABLE #temp_tbl AS (\n$0\n)\nWITH DATA PRIMARY INDEX( s) ON COMMIT PRESERVE ROWS;",
                ...
                panel.run_command(
                    "append",
                    {
                        "characters": f"Created init \nEDW_SQL.sublime-completions in\n{plugin_path}\nCreated init\nEDW_SQL.custom-completions,\nEDW_SQLAlias.custom-completions,\nEDW_SQL.connection-group-list in \n{metastore_path}"
                    },
                )
                deleted = []
                for file in os.listdir(metastore_path):
                    if file.endswith(".connection-group") or file.endswith(
                        ".drop-down"
                    ):
                        deleted.append(file)
                ...
                completions, all_lst = conn.get_all_accessible_meta()
    ...
    def on_done(self, tbl_to_pull):
        def main_func(self, tbl_to_pull):
            ...
            try:
                ...
                panel.set_read_only(True)
                input_panel = self.window.show_input_panel(
                    "Enter Connection Group Name", "", on_execute, None, None
                )
...
class MetaSelectConnection(sublime_plugin.WindowCommand):
    def run(self):
        ...
        print(conn_lst)
        self.window.show_quick_panel(to_show, self.on_done, 0, 0)
